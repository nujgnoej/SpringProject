...
<div class="container" style="padding-top:100px; min-height:767px; min-width:1300px;">
	<div style="margin:auto;" class="text-center mb-5"><h3>문의글리스트</h3></div>
			...
			<tr>
				...
				<td class="text-left">
					<c:if test="${vo.openSw == '공개'}">
						<a href="${ctp}/admin/qnaContent?idx=${vo.idx}&pag=${pageVO.pag}&pageSize=${pageVO.pageSize}">${vo.title}</a>
					</c:if>
					...
					<c:if test="${(vo.openSw == '비공개' && vo.mid == sMid) || (vo.openSw == '비공개' && sLevel == 1)}">
						<img src="${ctp}/images/lock.png" width="19px" height="19px" style="margin-bottom:4px;"/>&nbsp;<a href="${ctp}/admin/qnaContent?idx=${vo.idx}&pag=${pageVO.pag}&pageSize=${pageVO.pageSize}">${vo.title}</a>
					</c:if>
				</td>
				<td>${vo.name}</td>
				<td>
