...
<head>
	...
	<script src="https://kit.fontawesome.com/95f99fa6fb.js" crossorigin="anonymous"></script>
	...
	<script>
		...
		function qnaDelete(idx, flag) {
			...
			if(${replyVosSize} > 1) {
				alert("해당글에 댓글이 있으므로 삭제할 수 없습니다.")
				return false;
			}
			else if(ans) {
				location.href="${ctp}/admin/qnaDelete?idx="+idx+"&pag="+pag+"&pageSize="+pageSize;
			}
		}
	    ...
	    function replyDelCheck(idx, qnaIdx) {
	    	...
	    	$.ajax({
	    		type  : "post",
	    		url   : "${ctp}/admin/qnaReplyDeleteOk",
	    		data  : {
	    			idx : idx,
	    			qnaIdx : qnaIdx
	    			},
	    ...
	    function replyCheck2(idx, level, levelOrder) {
	 	...
	 	function qnaList() {
	 		let pag = '${pag}';
	 		let pageSize = '${pageSize}';
	 		location.href = '${ctp}/admin/qnaList?pag='+pag+'&pageSize='+pageSize;
	 	}
	</script>
	<style>
		#mainTable td {
...
<div class="container" style="padding-top:100px; min-height:700px;">
	...
	<table class="table mb-2 text-center" id="mainTable">
		...
		<tr>
			<td colspan="2" style="padding:30px; height:200px; vertical-align:top !important;">${fn:replace(vo.content,newLine,"<br/>")}</td>
		</tr>
	...
	<div class="text-center mt-3">
		...
		<c:if test="${vo.mid == sMid || sLevel == '1'}">
			<input type="button" value="문의글삭제" class="btn btn-outline-danger" onclick="qnaDelete('${vo.idx}')"/>
		</c:if>
	</div>
	<!-- 답변출력 -->
					...
					<tr>
						...
						<td class="text-left">
							...
							<c:if test="${sMid == replyVo.mid || sLevel == 1}">
								<a href="javascript:replyDelCheck(${replyVo.idx},${replyVo.qnaIdx})" title="삭제하기"><i class="fa-solid fa-xmark"></i></a>
							</c:if>
						</td>
						<td class="text-left">
